...
# -*- coding: utf-8 -*-
"""
Interactive KPROJ unfolded band scatter plot with separate UP/DOWN sliders.
...
Run:
  python plot_kproj_spin_gui_two_sliders.py

Optional:
  python plot_kproj_spin_gui_two_sliders.py --emin -2 --emax 2
  python plot_kproj_spin_gui_two_sliders.py --up band_kproj_1.dat --down band_kproj_2.dat
"""
...
def main():
    parser = argparse.ArgumentParser(description="Plot KPROJ unfolded spin bands with two sliders.")
    ...
    parser.add_argument("--emin", type=float, default=None, help="Minimum energy/y value")
    parser.add_argument("--emax", type=float, default=None, help="Maximum energy/y value")
    ...
    plt.subplots_adjust(left=0.10, right=0.82, bottom=0.24, top=0.94)
    ...
    if args.emin is not None or args.emax is not None:
        ...
        ax.set_ylim(args.emin if args.emin is not None else ymin,
                    args.emax if args.emax is not None else ymax)

    # Slider axes.
    ax_up_slider = plt.axes([0.15, 0.13, 0.58, 0.03])
    ax_dn_slider = plt.axes([0.15, 0.08, 0.58, 0.03])
